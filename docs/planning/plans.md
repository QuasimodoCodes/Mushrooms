...
* **The Catch:** If a mushroom is damaged, missing parts, or occluded by mud, the model will naturally output a *low* confidence score, which flags the system that visual identification is unreliable.
...
* **The Action:** Build a risk framework that ingests the vision model's confidence score, the context validation, and the LLM reasoning.
* **The Goal:** Provide a final risk assessment. If the confidence score is low (because the mushroom was damaged) or the context doesn't match, the system overrides the label and flags the result as unreliable.
* **The Output:** A clear risk level, warning, and actionable recommendation (e.g., "Confidence is too low due to visual damage; do not ingest").
...
* **The Goal:** Deploy the final, working application.
