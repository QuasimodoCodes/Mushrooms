"""
Train YOLOv10n-cls on the mushroom dataset.

Follows the exact same structure as scripts/training/yolo/train_yolo.py
but uses the YOLOv10 nano classification backbone.

YOLOv10 vs YOLOv8/v26
-----------------------
YOLOv10 (Wang et al., 2024) introduces NMS-free training via dual-label
...
def main():
    ...
    # ── 2. Load pretrained YOLOv10n classification backbone ──────────────────
    # 'yolov10n-cls.pt' is downloaded automatically from Ultralytics HUB on
    # first run (requires internet). The nano variant has ~2.3M parameters —
    # similar size to YOLOv8n-cls, but trained with the v10 dual-assignment head.
    ...
    print("Watch 'loss' decrease — early stopping triggers after 10 flat epochs.\n")
    ...
    results = model.train(
        ...
        dropout=0.0,           # YOLOv10 cls head already has BN; no extra dropout
    ...
    print(f"  Outputs saved  : docs/yolo10_runs/yolo10_classifier_v1/")
