...
def main():
    ...
    import shutil
    tflite_weights_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "docs", "yolo_runs", "yolo26_tflite", "weights")
    )
    os.makedirs(tflite_weights_dir, exist_ok=True)
    saved_model_dir = os.path.join(os.path.dirname(weights_path), "best_saved_model")
    for f in os.listdir(saved_model_dir):
        if f.endswith(".tflite"):
            src = os.path.join(saved_model_dir, f)
            dst = os.path.join(tflite_weights_dir, f)
            shutil.copy2(src, dst)
            print(f"  Copied: {f} -> yolo26_tflite/weights/{f}")
