...
def plot_confusion_matrix(matrix, class_names, save_path, title="ConvNeXt-Tiny"):
    norm = matrix.float()
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate a trained ConvNeXt-Tiny run")
    parser.add_argument("--run", required=True,
                        help="Run folder inside docs/herman_runs/ (e.g. convnext_tiny_adamw_ce_smooth)")
    ...
    print("=========================================")
    print("  Evaluating ConvNeXt-Tiny on Test Set")
    print(f"  Run: {args.run}")
    print("=========================================")

    if not os.path.exists(best_pt):
        print(f"ERROR: No weights at {best_pt}")
        print("       Run Herman/convnext/train.py first.")
        return
    ...
    plot_confusion_matrix(matrix, class_names, cm_path, title="ConvNeXt-Tiny")

    # Top errors
    errors = find_top_errors(matrix, class_names, n=20)
    errors_path = os.path.join(run_dir, "top_errors.txt")
    with open(errors_path, "w") as f:
        f.write(f"Top 20 most-confused pairs (ConvNeXt-Tiny test set)\n")
    ...
    with open(pc_path, "w") as f:
        f.write(f"Per-class accuracy — ConvNeXt-Tiny  (Top-1: {top1:.2f}%)\n")
        f.write("-" * 60 + "\n")
